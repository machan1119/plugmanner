...
const SectionWhyChooseItems = memo(({ item }: SectionWhyChooseItemsProps) => {
  return (
    <div className="flex flex-col md:flex-row grow w-full h-full p-2 items-center bg-white border-black-dark text-black rounded-[16px] hover:shadow-lg transition-all duration-300 border border-black-dark/50">
      ...
      <div className="flex flex-col md:w-[60%] w-full justify-between text-center md:text-left">
        <h3 className="font-clash font-semibold text-[18px] sm:text-[20px] leading-[25px] mb-1 w-full animate-fade-in">
          {item.title}
        </h3>
        <span className="text-black/50 text-[14px] sm:text-[16px] leading-6 font-satoshi w-full text-center md:text-left animate-fade-in-up">
          {item.description}
        </span>
      </div>
    </div>
